...
- **AI交易咨询**：与DeepSeek V4大模型进行交易策略讨论和市场分析，支持真正的流式逐字输出
...
- 2026年7月23日
  - **DeepSeek V4 模型升级**：替换已下架的旧模型（R1/Chat/Coder），改为 `deepseek-v4-pro` 和 `deepseek-v4-flash`
  - **真正的流式输出**：DeepSeek 对话支持逐字流式输出，移除"正在生成回复"占位，响应即时可见
  - **多轮对话优化**：对话历史持久化，支持连续深入探讨，自动管理上下文长度
  - **股票收藏功能**：点击 ☆ 即可收藏/取消收藏股票，常用列表即时更新，支持一键加载和删除
  - **DeepSeek 股票选择器**：对话中可选择已加载的任意股票数据发送给 AI 分析
  - **前端资源离线化**：Bootstrap CSS 下载到本地，不再依赖境外 CDN，国内网络无需代理
  - **UI 交互优化**：
    - 搜索框瘦身，按回车即可获取数据，移除冗余的"获取数据"按钮
    - "更新数据"和"收藏"按钮移到图表标题栏右侧，与 K 线开关并列
    - 数据加载时显示转圈动画，状态清晰可见
    - 默认股票改为 300502（新易盛）
